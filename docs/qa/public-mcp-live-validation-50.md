...
배포된 공개 `public read-only` MCP/API를 2026-03-15 KST 기준으로 실제 점검한 운영 시트입니다. 이번 실측은 **public API 우선** 방식으로 실행했고, `expected_mcp_flow`는 ChatGPT/Codex에서 이상적으로 밟아야 하는 MCP 흐름을 함께 기록합니다.
...
| LV011 | RC01 | 2026년 1학기 데이터베이스 과목 찾아줘 | course | `prompt_search_courses -> tool_search_courses` | public course snapshot | completed | `/courses?query=데이터베이스&year=2026&semester=1`이 빈 배열을 반환한다. | year/semester 파라미터는 전달됐지만 현재 공개 snapshot에서 대표 과목 질의를 소화하지 못한다. | soft_fail | 현재 공개 DB의 과목 coverage가 CS 대표 질의 기준으로 얕다. |
| LV012 | RC02 | 2026년 1학기 객체지향 과목 찾아줘 | course | `prompt_search_courses -> tool_search_courses` | public course snapshot | completed | `/courses?query=객체지향&year=2026&semester=1`이 빈 배열을 반환한다. | 대표 과목명 질의가 빈 결과라 릴리즈팩 기대를 충족하지 못한다. | soft_fail | 현재 snapshot에 객체지향 계열 과목이 보이지 않는다. |
| LV013 | RC03 | CSE301 과목 뭐야 | course | `tool_search_courses` | public course snapshot | completed | `/courses?query=CSE301`이 빈 배열을 반환한다. | code query는 깨지지 않았지만 공개 snapshot에서 해당 과목을 검증하지 못한다. | soft_fail | 대표 코드 검색 happy path가 현재 배포 데이터로는 증명되지 않는다. |
| LV014 | RC04 | 김가톨 교수 수업 보여줘 | course | `tool_search_courses` | public course snapshot | completed | `/courses?query=김가톨`이 빈 배열을 반환한다. | 교수명 기반 질의를 live snapshot으로 검증하지 못한다. | soft_fail | 현재 공개 snapshot에 해당 교수 데이터가 없거나 query coverage가 부족하다. |
| LV015 | RC05 | 2026년 1학기 데이터베이스인데 김가톨 교수 수업만 보여줘 | course | `prompt_search_courses -> tool_search_courses` | public course snapshot | completed | `/courses?query=데이터베이스 김가톨&year=2026&semester=1`이 빈 배열을 반환한다. | 복합 필터 경로는 깨지지 않았지만 유효 결과를 검증하지 못한다. | soft_fail | year/semester + title + professor를 함께 다루는 대표 질의가 비어 있다. |
| LV016 | RC06 | 객체지향 과목 2개만 보여줘 | course | `tool_search_courses` | public course snapshot | completed | `/courses?query=객체지향&limit=2`가 빈 배열을 반환한다. | limit 파라미터는 전달됐지만 결과가 없어 체감 동작을 검증하지 못한다. | soft_fail | limit 회귀를 확인하려면 먼저 대표 과목이 snapshot에 있어야 한다. |
| LV017 | RC07 | 객체 지향 과목 찾아줘 | course | `prompt_search_courses -> tool_search_courses` | public course snapshot | completed | `/courses?query=객체 지향&year=2026&semester=1`이 빈 배열을 반환한다. | spacing recovery를 보여주지 못했고, empty snapshot으로 종료됐다. | soft_fail | place와 달리 course spacing recovery는 현재 공개 데이터로 검증되지 않는다. |
| LV018 | RC08 | 데이타베이스 과목 있어 | course | `prompt_search_courses -> tool_search_courses` | public course snapshot + 제품 alias 정책 | completed | `/courses?query=데이타베이스&year=2026&semester=1`이 빈 배열을 반환한다. | 제품성 typo alias를 live snapshot에서 증명하지 못한다. | soft_fail | typo normalization 여부보다 현재 course coverage 문제가 먼저 드러난다. |
| LV019 | RC09 | CSE 420 과목 뭐야 | course | `tool_search_courses` | public course snapshot | completed | `/courses?query=CSE 420`이 빈 배열을 반환한다. | code spacing recovery가 확인되지 않는다. | soft_fail | course code query 품질을 판정할 표본이 현재 공개 snapshot에 없다. |
| LV020 | RC10 | 7교시에 시작하는 과목 찾고 싶어 | course | `songsim://class-periods -> tool_search_courses` | class-periods resource + public course snapshot | completed | `/periods`가 1~10교시 표를 반환하고, `/courses?year=2026&semester=1&limit=10`에는 `3D애니메이션1`처럼 7교시 시작 과목이 포함된다. | 교시표와 course snapshot은 존재하지만 API에 `period_start=7` 직접 필터는 없어 resource chaining 전제가 필요하다. | soft_pass | MCP에서는 `songsim://class-periods`를 읽고 course search 결과를 좁히는 식으로 설명 가능하다. |
...
| LV026 | RN06 | 학사 공지 5개 최신순으로 요약해줘 | notices | `prompt_latest_notices -> tool_list_latest_notices` | 공식 notice source_url | completed | `/notices?category=academic&limit=5`와 `/gpt/notices?category=academic&limit=5`가 모두 빈 배열을 반환한다. | 현재 공식 사이트에는 2026년 3월 학사 공지가 보이는데 public snapshot에서는 academic slice가 비어 있어 릴리즈 기대를 충족하지 못한다. | fail | notice ingest 범위나 category mapping을 다시 봐야 한다. |
...
| LV035 | RR07 | 학생식당 기준 지금 여는 카페만 3개 | restaurants | `prompt_find_nearby_restaurants -> tool_find_nearby_restaurants` | Kakao Maps public listing | completed | `/restaurants/nearby?origin=학생식당&open_now=true&category=cafe&limit=3`이 3건을 반환하지만 각 item의 `open_now`는 모두 `null`이다. | 필수 제약인 `open_now`를 실측 payload에서 보장하지 못해 현재 시점 영업 중인 카페만 골랐다고 보기 어렵다. | fail | restaurant hours/source coverage가 더 필요하다. |

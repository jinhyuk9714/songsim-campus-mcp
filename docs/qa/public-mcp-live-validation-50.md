...
| LV020 | RC10 | 7교시에 시작하는 과목 찾고 싶어 | course | `songsim://class-periods -> tool_search_courses(period_start=7)` | class-periods resource + public course snapshot | completed | `/periods`가 1~10교시 표를 반환하고, `/courses?year=2026&semester=1&period_start=7&limit=10`은 `3D애니메이션1`을 포함한 7교시 시작 과목만 직접 반환한다. | 교시 metadata와 course search가 이제 direct filter로 바로 연결된다. | pass | `period_start` optional filter를 추가한 뒤 resource chaining이 즉답형 공개 경로로 바뀌었다. |

...
| 학사지원 | `https://www.catholic.ac.kr/ko/support/calendar2024_list.do` | 부분적으로 가능. `학사일정`, `업무안내`, `학적변동(복학/자퇴/재입학)`, `등록 안내`, `수업 안내`, `계절학기`, `증명발급`, `장학제도`, `휴학 안내`, `개설과목`, `교시표`, `학사 관련 공지`, `예상 빈 강의실`은 직접 질의 가능 | 부분지원 | [docs/source_registry.md](../source_registry.md) 의 `cuk_academic_calendar`, `cuk_academic_support_guides`, `cuk_academic_status_guides`, `cuk_registration_guides`, `cuk_class_guides`, `cuk_seasonal_semester_guides`, `cuk_certificate_guides`, `cuk_leave_of_absence_guides`, `cuk_scholarship_guides`, `cuk_subject_search`, `cuk_campus_notices`; [docs/connect-codex.md](../connect-codex.md); [src/songsim_campus/api.py](../../src/songsim_campus/api.py) 의 `/academic-calendar`, `/academic-support-guides`, `/academic-status-guides`, `/registration-guides`, `/class-guides`, `/seasonal-semester-guides`, `/certificate-guides`, `/leave-of-absence-guides`, `/scholarship-guides`, `/courses`, `/periods`, `/notices`, `/classrooms/empty`; [src/songsim_campus/mcp_server.py](../../src/songsim_campus/mcp_server.py) 의 `tool_list_academic_calendar`, `tool_list_academic_support_guides`, `tool_list_academic_status_guides`, `tool_list_registration_guides`, `tool_list_class_guides`, `tool_list_seasonal_semester_guides`, `tool_list_certificate_guides`, `tool_list_leave_of_absence_guides`, `tool_list_scholarship_guides`, `tool_search_courses`, `tool_get_class_periods`, `tool_list_latest_notices`, `tool_list_estimated_empty_classrooms` | 남은 큰 갭은 `학생교류`입니다 | `exchange_*.do` 정적 안내 파서 추가 |

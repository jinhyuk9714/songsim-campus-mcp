...
1. `학생활동` family v1 정적 4페이지: `총학생회`, `교내미디어`, `사회봉사`, `학생군사교육단`
2. `대학생활`의 학생지원 세부 페이지 파서
3. `생활지원`의 후속 확장: 보건실/유실물/주차 외 세부 지원 도메인과 시설 micro-location 강화
4. `기숙사`의 홈 카드 이후 상세 board / 본문 검색 확장
5. `가대소개`의 주요 정적 안내 resource
6. `CUK홍보`의 `보도자료` 또는 `포토뉴스` 목록 파서
7. `서비스이용안내`의 `입찰공고`, `채용공고` 목록 파서

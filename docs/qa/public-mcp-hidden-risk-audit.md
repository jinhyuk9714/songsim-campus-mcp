...
| CW05 | CSE 420 과목 뭐야 | `GET /courses?query=CSE%20420&year=2026&semester=1&limit=5` | spacing normalization이 `CSE420` query group과 정렬돼야 한다 | `[]` | fast | pass | `normalized_spacing` | normalization은 적용되지만 source-backed direct hit는 아직 없다 |
...
- `course`는 release gate가 아니라 watchlist라는 현재 정책을 유지한다. `데이터베이스`와 `데이타베이스`는 near-match recovery로 기록하고, `CSE 420`은 normalized spacing이 적용된 source-gap watch로 남긴다. `CSE301`과 `김가톨`도 source-backed 여부 중심으로 계속 추적한다.
